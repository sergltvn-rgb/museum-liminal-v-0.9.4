...
Версии идут сверху вниз от новой к старой. Ранние записи (0.2.0–0.8.5) сохранены как есть,
на английском, в том виде, в каком их писали.

## Не выпущено

Работа в ветке после того, как был проставлен тег 1.0.0. Проверено чтением кода и прогоном
`run_tests.cmd`; в сборку ничего из этого ещё не уходило.

- Дев-аддон `addons/godot_ai` и тестовые скрипты исключены из всех трёх экспортных пресетов
  (`exclude_filter`). Раньше в релиз попадали ~137 скриптов MCP-сервера, который поднимает
  HTTP-порт и спавнит процессы.
- ИИ Куратора вынесен в `game/CuratorMonster.gd` — отдельный `CharacterBody3D` с собственным
  `_physics_process`. До этого `move_and_slide()` вызывался из `_process`, и на 144 Гц монстр
  был примерно вдвое быстрее, чем на 60.
- Добавлена настоящая навигация: `FirstMuseumMap._add_navigation()` печёт `NavigationRegion3D`
  по статическим коллайдерам. Раньше `NavigationAgent3D` создавался и не использовался ни разу,
  а `NavigationRegion3D` в проекте не было — Куратор шёл по прямой и застревал в стенах.
- `unlock_wing()` теперь отсоединяет гермодверь от дерева до перепечки навмеша, иначе
  отложенный `queue_free()` оставлял крыло запертым для навигации.
- Локализация переведена на единый каталог: 398 ключей в `localization/game.csv`, все
  используются кодом. Добавлен `tools/check_localization.py` — ловит запрошенный ключ,
  которого нет, и строку, где `%`-спецификаторы расходятся с вызовом. Добавлен `game/Loc.gd`
  — безопасная замена `tr(KEY) % args`.
- `game/trials/RiftTrial.gd` перестроен с фасада на реальный контракт из семи хуков
  (`_build`, `_tick`, `_tick_visuals`, `_on_interact`, `_on_secondary`, `_on_fall`,
  `_teardown`). Из прежних десяти методов вызывались четыре, а `reset_after_fall()` уводил
  в несуществующий метод и уронил бы первого, кто его подключит.
- Добавлен пятый набор тестов `game/test_blocker_regressions.gd`: проходимость дверного
  проёма Атриум → Time Wing B, восстановление после падения за плоскость смерти, ограждение
  площади перед музеем.
- `run_tests.cmd` научился находить Godot по переменной `GODOT`, а не только по PATH,
  проверяет код возврата шага импорта и поддерживает `SKIP_IMPORT=1`.
- В CI добавлены шаги «Blocker regressions» и «Localization catalogue».
- Из настроек убраны переключатели «Субтитры» и «Уменьшение движения»: оба хранились
  в конфиге и не читались ни одним скриптом. Переключатель доступности, который ничего
  не меняет, хуже отсутствующего. Строки каталога сохранены на будущее, разбор — в
  комментарии `game/SettingsManager.gd:23`.

## 1.0.0

Сборка контента: пролог, десять измерений, RU/EN, CI.

- Добавлен отдельный интерактивный пролог с проверкой движения, обзора, спринта, прыжка,
  фонаря, взаимодействия и выхода.
- Экран провала показывает причину, прогресс ночи и контекстный совет для конкретного измерения.
- RiftTrialManager разделён на диспетчер, реестр и 10 сцен-дескрипторов испытаний.
- Подключены все поставленные GLB/FBX-модели с процедурными fallback и автоколлизиями.
- Проведён единый арт-, аудио- и сюжетный проход: сектор ориентации, музейный архив моделей,
  усиленные звуковые статусы и связная рамка смены.
- Добавлены RU/EN-каталог, переключение языка, высокий контраст, уменьшение мерцания и крупный UI.
- Добавлен CI (`.github/workflows/windows-release.yml`): импорт, smoke/map/catalog/full-cycle
  тесты и шаг кросс-экспорта Windows EXE.
- Windows-профиль получил версию 1.0.0, встроенный PCK и релизные метаданные.

**Поправка, 2026-07-27.** Первоначальный текст этой записи назывался «Полный Windows-релиз»
и утверждал, что EXE собран. Это было неправдой на момент написания и остаётся неправдой
сейчас: `--export-release` не выполнялся ни разу. Все 10 исторических запусков CI падали на
шаге «Map verification», а шаги «Export Windows release» и «Upload Windows artifact»
получали статус `skipped`. Причина падения с тех пор устранена, но выпущенной сборки нет.
Заявленные в этой записи субтитры и уменьшение движения из списка убраны: системы субтитров
в проекте никогда не было, а оба переключателя с тех пор удалены как фиктивные
(см. раздел «Не выпущено» и README, «Состояние проекта»).

## 0.9.4

### Исправлено
- Тестовая консоль (F9): кнопки теперь нажимаются — на время открытой консоли отключается
  управление игроком и перехват мыши.

### Добавлено
- Новый UI консоли: карточки измерений с описаниями, цветовые акценты, hover-эффекты, кнопка
  закрытия. После теста состояние ночи восстанавливается как было.
- Зал отражений: силуэты в зеркалах (дефектные дрожат), люстра, 3 ошибки — зал перестраивается.
- Жёлтые залы: Блуждающий (патруль/преследование, отбирает плёнку), световые панели,
  жёлто-чёрная разметка выхода.
- Служба извлечения: дрон-сканер с красным лучом (отбирает ношу), ценник в у.е., экспонат
  виден в руках.
- Восхождение: контрольные точки (зелёные плиты), 3 движущихся уступа, маяк вершины; туман
  откатывается к чекпоинту.

## 0.9.3

### Добавлено
- 4 новых измерения: Зал отражений, Жёлтые залы (Backrooms), Служба извлечения
  (R.E.P.O. / Lethal Company), Восхождение (PEAK). Всего 10 аномалий.
- Новый экран «Протокол локализации»: панель с цветовым акцентом аномалии, назначением
  прибора и статусом (E/Enter — закрыть, автозакрытие 10 с).

### Изменено
- Эхо-камера: фаза осмотра 7 с, явные вспышки (ярче + свет + звук, паузы между ними), при
  ошибке показ повторяется с указанием шага.
- Тёмный сектор: стены до 48 м, узлы и выход полностью скрыты — видны только точками лидара
  и слабым отсветом вблизи.

## 0.9.2

Стабильность и новые измерения.

- Исправлен вылет после завершения ТЁМНОГО СЕКТОРА (обращение к global_position после очистки
  измерения).
...
- НОВОЕ ИЗМЕРЕНИЕ «ЗЫБКИЙ МОСТ» (аномалия «Фазовый провал», ключ — Фазовая призма):
  6 рядов плит, в каждом одна фантомная: наступили — возврат на старт. Пройденные плиты
  подсвечиваются.
...
- ТЁМНЫЙ СЕКТОР переделан: полная темнота, лидар-скан на ЛКМ расставляет до 4200 светящихся
  точек (синие — стены, голубые — узлы, зелёные — выход, красные — монстр). R — мини-карта.
- Склад: верстаки разбиты на 4 сегмента у стен, центральный проход между дверями свободен.
- У каждого из 12 приборов теперь есть пассивный эффект (см. README, «Приборы»).
- Убраны предупреждения о неиспользуемом delta.

## 0.9.0

- Переработано движение: плавный разгон, управление в воздухе, coyote time, буфер и высота
  прыжка, автоматический подъём на ступени, безопасная телепортация.
