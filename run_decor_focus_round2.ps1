...
$outDir = Join-Path $root 'shots\decor_audit\focus_round2'
if (Test-Path $outDir) { Remove-Item -Recurse -Force $outDir }
$outLog = Join-Path $root 'shots\decor_audit\focus_round2.stdout.log'
$errLog = Join-Path $root 'shots\decor_audit\focus_round2.stderr.log'
$pidFile = Join-Path $root 'shots\decor_audit\focus_round2.pid'
Remove-Item $outLog,$errLog,$pidFile -ErrorAction SilentlyContinue
$env:DECOR_CONFIG = 'res://decor_focus_round2.json'
