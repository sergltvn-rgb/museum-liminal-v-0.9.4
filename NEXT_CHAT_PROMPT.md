...
| ~~35~~ **ОТКАЗ** | `ExteriorProps.build_player_car` | салон POV-катсцены: стёкла, эмиссия, наклоны под кадр — подробно в п. 4 |
...
4. ~~**Стойка лобби**~~ **СДЕЛАНО** 2026-08-10, коммит `0925a63`.
   ...
   **Машина игрока (35) — ОТКАЗ, вопрос закрыт, повторно не планировать.**
   `ExteriorProps.build_player_car` — это салон POV-катсцены, камера сидит внутри
   (`DRIVER_EYE := Vector3(-0.37, 1.08, 0.12)`, `ExteriorProps.gd:94`,
   `FirstMuseumMap.gd:3855`); шесть прозрачных примитивов на `COL_GLASS_TINT` с альфой
   0.35 и четыре эмиссивных (фары и стопы 0.35, приборный диск 0.6, радио 0.4)
   дом-стиль с одним непрозрачным атласом и `metallic 0` просто убьёт; геометрия
   наклонена под кадр (лобовое и A-стойка −28°, C-стойка +24°, колонка и руль 65°).
   Максимум, на что машина годится в будущем — внешняя оболочка в модель, стёкла
   и салон процедурно, приёмка из кабины.
