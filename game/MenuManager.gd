...
## каждой смене ночи.
##
## Сразу за записью идёт _start_game(): кнопка, которая только ставит флаг,
## выглядела бы как кнопка, которая ничего не делает.
##
## И ИМЕННО ЭТИМ ОНА ВЫГЛЯДЕЛА — та самая жалоба «первое нажатие впустую,
## да и вообще проигрывается не всегда». Комментарий выше был неверен: карта
## не загружается по нажатию, она уже стоит в дереве под паузой за главным
## меню, и её _ready() вместе с _start_opening() отработал ещё до того, как флаг
## лёг на диск. _start_game() только снимает паузу и ничего не перечитывает,
## так что заезд ждал следующей загрузки карты.
##
## Поэтому флаг теперь не единственный механизм, а страховка: живой карте
## говорят играть сразу (replay_drive()), а если карты в дереве нет или она
## не готова играть (идёт другая катсцена, нет игрока или машины) — флаг
## остаётся лежать и срабатывает по-старому, при следующей загрузке. Снимает
## его в обоих случаях один и тот же _on_drive_finished().
func _rewatch_drive() -> void:
	...
	_start_game()
	# После _start_game(), а не до: Cutscene строится с PROCESS_MODE_INHERIT и на
	# паузе не потратила бы ни кадра, а игрок увидел бы замерший первый
	# кадр заезда ещё под меню.
	var map := get_tree().get_first_node_in_group("museum_map")
	if map != null and map.has_method("replay_drive"):
		map.call("replay_drive")
